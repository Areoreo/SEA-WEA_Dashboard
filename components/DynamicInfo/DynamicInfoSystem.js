...
export const DynamicInfoPanel = ({ reservoir, onClose, onZoomToReservoir }) => {
    ...
    const loadTimeSeriesData = async (index, commissionYear = null) => {
        ...
        try {
            ...
            if (!csvData) {
                throw new Error(
                    `Unable to find time series data files for INDEX ${index}. Tried: ${possibleFiles.join(
                        ", "
                    )}`
                );
            }
            ...
            console.log(`Loaded ${parsedData.length} data points from ${loadedFile}`);
    ...
    // Define charts configuration with English labels
    const charts = [
        {
            key: "elevation",
            title: "Water Level (m)",
        ...
        return (
            <div className="h-full bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-600">Loading dynamic data...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="h-full bg-white flex items-center justify-center p-6">
                <div className="text-center max-w-lg">
                    <div className="mb-4">
                        {/* <BarChart3 className="w-16 h-16 mx-auto mb-4 opacity-50" /> */}
                        <h3 className="text-lg font-semibold mb-2">Unable to Load Dynamic Data</h3>
                    ...
                    {/* <div className="bg-blue-50 p-4 rounded-lg text-left text-sm">
                        ...
                        <ul className="text-blue-700 space-y-1">
                            <li>• Reservoir: {reservoir.reservoir_name}</li>
                            <li>• INDEX: {reservoir.INDEX}</li>
                            <li>• Commission Year: {reservoir.commission_year || "N/A"}</li>
                        </ul>
                    </div> */}
    ...
    return (
        <div className="h-full bg-white flex flex-col">
                ...
                <div className="flex justify-between items-start">
                    <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 mt-2 text-sm text-gray-600">
                            ...
                            {filteredData && <span>{filteredData.length} data points</span>}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <button
                            onClick={onClose}
                            className="px-3 py-1 bg-gray-500 text-white text-sm rounded hover:bg-gray-600 transition-colors inline-flex items-center space-x-1"
                        >
                            <X className="w-3 h-3" />
                            <span>Close</span>
                        </button>
            ...
            {/* UPDATED: Charts section - horizontal layout, narrower charts */}
            <div className="flex-1 p-4">
                <div className="flex space-x-3 h-full">
                    {charts.map((chart, index) => (
                        <div
                            key={chart.key}
                            className="flex-1 bg-gray-50 rounded-lg border border-gray-200 p-3 min-w-0"
                        >
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="text-xs font-medium text-gray-700 flex items-center space-x-1">
                                    <chart.icon className="w-3 h-3 flex-shrink-0" />
            ...
            {/* REMOVED: Summary cards section - completely removed as requested */}
        </div>
    );
};
