...
const MapComponent = ({
    ...
    // // Create individual marker icons
    ...
    //     // Size based on attribute value (min 8px, max 24px)
    //     const minSize = 8;
    //     const maxSize = 24;
    ...
    //     });
    // };
    // BELOW is a new createIndividualIcon which can show different types when isSummary==true and activeTab=='main_use'
    const createIndividualIcon = (item, attributeValue) => {
        const category = categorizeReservoirData(item, activeTab, selectedAttributes[0]);
        const baseColor = getCategoryColor(category);
